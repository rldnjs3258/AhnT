...
public class GameState implements IState {
    ...
    @Override
    public void Init() {
        m_Sound_Background= MediaPlayer.create(AppManager.getInstance().getGameView().getContext(),R.raw.gameover);
        m_Sound_Background.start( );

        //m_SoundPool =new SoundPool(2, AudioManager.STREAM_MUSIC,0);
        //m_sound_shoot = m_SoundPool.load(AppManager.getInstance().getGameView().getContext(),R.raw.gun,1);
    ...
    public void Render(Canvas canvas) { //그리는 순서 조심하기.
        ...
        Paint p = new Paint();
        p.setTextSize(60); //글씨 크기
        p.setColor(Color.WHITE); //글씨 색
        canvas.drawText("남은 목숨:"+String.valueOf(m_player.getLife()), 0, 70, p);
        Paint p2 = new Paint();
        p.setTextSize(60); //글씨 크기
        p.setColor(Color.WHITE); //글씨 색
        canvas.drawText("얻은 점수:"+String.valueOf(m_point), 0, 160, p);
        ...
        if ((CurrentTime - System.currentTimeMillis())/1000 < 0){
            ...
            AppManager.getInstance().getthread().setRunning(false);
            AppManager.getInstance().getgameActivity().change();
    ...
    public void Update() {
        ...
        for(int i = m_itemlist.size() -1; i>=0; i--){
            ...
            if(item.bOut==true)m_itemlist.remove(i);//일정 시간 후 아이템 사라짐
        }
        if (System.currentTimeMillis() - LastShoot >= 500){ //시스템 시간 밀리초 마다 실행되게 하기
            LastShoot = System.currentTimeMillis();
            //미사일 발사 로직
            m_pmslist.add(new Missile_Player(m_player.getX()+30,m_player.getY()-100));
        }
    ...
    public void MakeItem(){
        if (System.currentTimeMillis() - LastItem >= 3000){ //일정 시간마다 아이템이 랜덤으로 생성
            LastItem = System.currentTimeMillis();

            int itemtype = randIt.nextInt(4);
            Item eit = null;
            if (itemtype == 0) eit = new ItemAddLife(randIt.nextInt(400), randIt.nextInt(600));
            else if (itemtype == 1) eit = new ItemBomb(randIt.nextInt(400), randIt.nextInt(600));
            else if (itemtype == 2) eit = new ItemAddTime(randIt.nextInt(400), randIt.nextInt(600));
            else if (itemtype == 3) eit = new ItemAddScore(randIt.nextInt(400), randIt.nextInt(600));

            m_itemlist.add(eit); //리스트에 적을 추가한다.
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_MOVE){ //마우스의 움직임이 있으면
            ...
            if (System.currentTimeMillis() - LastShoot >= 1000){ //시스템 시간 밀리초 마다 실행되게 하기
               ...
               // m_SoundPool.play(m_sound_shoot, 1, 1, 0, 0, 1);
            }
        ...
        for (int i = m_enemmslist.size()-1; i>=0; i--){ //적의 미사일 리스트를
            if (CollisionManager.CheckBoxToBox(// 트루/false 체크한다.
                ...
                if (m_player.getLife() <= 0) {
                    ...
                    // System.exit(0); //생명 0이면 종료 -> 스테이트 변화 등으로 바꿀 수 있음
            }
        }
        ...
        for (int i = m_pmslist.size()-1; i>=0; i--){ //플레이어의 미사일 리스트
            for (int j = m_enemylist.size()-1; j>=0; j--){ //적 리스트
                if(CollisionManager.CheckBoxToBox(m_pmslist.get(i).m_BoundBox, //플레이어 미사일 리스트와
                    ...
                    m_enemylist.get(j).hp-=10;
                    if(m_enemylist.get(j).hp<=0) {
                        m_point += m_enemylist.get(j).point;
